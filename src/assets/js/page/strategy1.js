(function ($) {
  var strategy = {
    ...
    init: function () {
      ...
      // strategy.timer11 = setInterval(
      //   function () {
      //     strategy.timerTf = false
      //     var type = $('.title').attr('data-type');
      //     if (type == '1' && document.getElementById('dingshiid1')) {
      //       strategy.getDetails()
      //     } else {
      //       window.clearInterval(strategy.timer11)
      //     }
      //   }, 10000);
    ...
    searchFn: function () {
      $('.search-box [name="keyword"]').bind('input porpertychange', function () {
        ajaxPost({
          ...
          data: { keyword: $.trim($('.search-box [name="keyword"]').val()), stock_type: 1 },
          success: function (res) {
            if (res.code == '200') {
    ...
    getDetails: function () {
      ...
      var stockCode = localStorage.getItem("stockCode1")||strategy.cfdListData[0].stock_code;
      var stock_code = strategy.keyword == '' ? stockCode : strategy.keyword;
      ajaxPost({
        url: '/v1_home/stock/detail',
        data: { stock_code: stock_code, type: strategy.sharesListType || 0, stock_type: 1 },
        success: function (res) {
          if (res.code == '200') {
            var data = res.data;
            localStorage.setItem('stockCode1', data.stock_code)
            if (data.market_id == '3') {
              $('.business-box').hide()
            } else {
              $('.business-box').show()
              ...
              utils.dialog({
                ...
                temp: '<div class="msg">' + res.msg + '</div>\
                    <div class="c-dialog-btns">\
                        <a class="commit-btn loadPage" href="#page/strategy1.html">确认</a>\
                    </div>',
    ...
    getBuyParameter: function () {
      ...
      ajaxPost({
        ...
        data: { stock_code: strategy.keyword || stockCode, demo_flag: '0', stock_type: 1 },
        success: function (res) {
          if (res.code == '200') {
          ...
          if (res.code == '200') {
            ...
            if (list.length > 0) {
              ...
              for (var i = 0; i < list.length; i++) {
                ...
                zhiHtml = '<span class="zhiyzhis" style="background-color:yellow;" data-type="' + i + '">止盈止损</span>'
                if (list[i].aoto_delay == '1') {
                  diyanHtml = '<span class="guanbidiyan" style="background-color:orange;" data-type="' + i + '">关闭递延</span>'
                } else {
                  diyanHtml = ''
                }
                var tbHtml = ''
                if (list[i].suspend_status == '0') {
                  tbHtml = '<span class="xiangqing" data-type="' + i + '"> <a class="loadPage" href="#page/strategyDetails.html?flagid=' + list[i].id + '&outid=1" data-type="' + i + '">详情</a></span><span class="maichu" style="background-color:#00FF7F;" data-type="' + i + '" data-cmd="' + list[i].cmd + '">终止合约</span>' + zhiHtml + '' + diyanHtml + ''
                } else {
                  zhiHtml = ''
                  diyanHtml = ''
                  tbHtml = '<span class="xiangqing" data-type="' + i + '"> <a class="loadPage" href="#page/strategyDetails.html?flagid=' + list[i].id + '&outid=1" data-type="' + i + '">详情</a></span><span class="maichu" style="background-color:#00FF7F;" data-type="' + i + '" data-cmd="' + list[i].cmd + '">赎回</span>' + zhiHtml + '' + diyanHtml + ''
                }
                var diyanHtml = '';
    ...
    getUpdateData:function(){
      // console.info(strategy.stock_key)
      ajaxPost({
    ...
    getkline: function () {
      ...
      ajaxPost({
        ...
        data: {
          keyword: stock_code, stock_type: 1,
