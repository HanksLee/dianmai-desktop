  ...
  var strategy = {
    ...
    init: function () {
      ...
      if (firstDomain == 'raytekasia.com') {
        strategy.detailBtnStyle = 'style="background-color: rgb(102, 177, 255);"'
      ...
      // strategy.timer22 = setInterval(
      //   function () {
      //     strategy.timerTf = false
      //     var type = $('.title').attr('data-type');
      //     if (type == '1' && document.getElementById('dingshiid2')) {
      //       strategy.getDetails()
      //     } else {
      //       clearInterval(strategy.timer22)
      //     }
      //   }, 10000);
        // strategy.timer222 = setInterval(
        //   function () {
    ...
    getList: function () {
      ajaxPost({
        url: '/v1_home/stock/cfdproduct',
        data: { stock_type: 2 },
        success: function (res) {
          if (res.code == '200') {
            strategy.cfdListData = res.data.page_data
            console.log(strategy.cfdListData)
            strategy.getDetails();
    ...
    //输入搜索
    searchFn: function () {
      $('.search-box [name="keyword"]').bind('input porpertychange', function () {
        ajaxPost({
          url: '/v1_home/stock/searchall',
          data: { keyword: $.trim($('.search-box [name="keyword"]').val()), stock_type: 2 },
          success: function (res) {
            if (res.code == '200') {
              var list = res.data.page_data || [];
              var sHtml, sHtmlData = [];
              strategy.sharesList = list;
              var classStr = '', market_name;
              if (list.length > 0) {
                ...
                for (var i = 0; i < list.length; i++) {
                  ...
                  if (list[i].market_id == '0'||list[i].market_id == '11') {
                    ...
                    market_name = 'SH';
                    key = '0';
                  } else if (list[i].market_id == '1' || list[i].market_id == '2') {
                    classStr = 'class="sz"';
                    market_name = 'SZ';
                    key = '0';
                  ...
                  sHtml = '<li data-type="' + stock_code + '" data-market="' + market_id + '" data-abbreviation="' + market + '" data-index="' + i + '">' +
                    '<span ' + classStr + '>' + market_name + '</span>' +
                ...
                $('.search-list').show();
                $('.search-list').html('<li class="tr-bg" style="text-align:center;color:#999;">暂无数据</li>');
              }
            }
          }
        })
    ...
    clickListSearchFn: function () {
      $('.search-list').on('click', 'li', function () {
        ...
        $('.search-box [name="keyword"]').val(stock_code);
        strategy.getDetails();
        strategy.getBuyParameter();
        $('.search-list').hide();
        $('.shares-type-ul').hide();
    ...
    getDetails: function () {
      ...
      var stockCode = localStorage.getItem("stockCode2")||strategy.cfdListData[0].stock_code;
      var stock_code = strategy.keyword == '' ? stockCode : strategy.keyword;
      ajaxPost({
        url: '/v1_home/stock/detail',
        data: { stock_code: stock_code, type: strategy.sharesListType || 0, stock_type: 2 },
        success: function (res) {
          if (res.code == '200') {
            var data = res.data;
            localStorage.setItem('stockCode2', data.stock_code)
            if (data.market_id == '3') {
              $('.business-box').hide()
            } else {
              $('.business-box').show()
            ...
            $('.now_price_box').text(res.data.margin.buy_price + '/' + res.data.margin.sell_price);
            ...
            if (strategy.timerTf) {
              utils.dialog({
                title: '提示',
                temp: '<div class="msg">' + res.msg + '</div>\
                    <div class="c-dialog-btns">\
                        <a class="commit-btn loadPage" href="#page/strategy2.html">确认</a>\
                    </div>',
    ...
    getBuyParameter: function () {
      var stockCode = strategy.cfdListData[0].stock_code;
      $('.create-a-strategy').hide();
      ajaxPost({
        url: '/v1_home/policy/buyfee',
        data: { stock_code: strategy.keyword || stockCode, demo_flag: '0', stock_type: 2 },
        success: function (res) {
          if (res.code == '200') {
            ...
            strategy.delay_fee = strategy.tList[0].delay_fee || '0';
            strategy.fee_time = strategy.tList[0].fee_time || '';
            strategy.delay_days = strategy.tList[0].delay_days || '';
            strategy.policy_time_text = strategy.tList[0].policy_time_text || '';
            ...
            // 点差二次修改开始
            $('.shoushu-input').val(strategy.tList[0].min_money);
            $('.min_shou').text(strategy.tList[0].min_money);
            $('.max_shou').text(strategy.tList[0].max_money);
            strategy.jisuanFn();
            // 点差二次修改结束
            ...
            strategy.operationFund = Number(strategy.tList[0].policy_money) * 10000 //操作资金
            if (listData.length > 0) {
              for (var i = 0; i < listData.length; i++) {
                var policy_name = listData[i].policy_name ? listData[i].policy_name : '';
                htmlTab = '<li>' + policy_name + '</li>';
                htmlTabData.push(htmlTab);
              };
              strategy.t_policy_leverage = listData[0].policy_leverage.split(",");
              $('.bond [name="bond"]').val((Number(listData[0].policy_money) * 10000));
            };
            if (htmlTabData.length > 0) {
              $('.product-tab ul').html(htmlTabData.join(''));
              $('.product-tab li').eq(0).addClass('active')
            };
            $('.bond .min').text(listData[0].min_money);
            $('.bond .max').text(listData[0].max_money);
            $('.strong-leveling-line em').text((strategy.organ.risk_margin_limit * 100) + '%');
            ...
            if (strategy.tList[0].etf_margin) {
              ...
              strategy.bondVal = Number($.trim($('.bond [name="bond"]').val())) === 0 ? 1 : Number($.trim($('.bond [name="bond"]').val()));
    ...
    tabTFn: function () {
      $('.product-tab').on('click', 'li', function () {
        ...
        strategy.t_policy_leverage = strategy.tList[index].policy_leverage.split(",");
        ...
        strategy.policy_time_text = strategy.tList[index].policy_time_text || '';
        strategy.tp_rate = strategy.tList[index].tp_rate || ''
        strategy.sl_rate = strategy.tList[index].sl_rate || ''
        strategy.operationFund = Number(strategy.tList[index].policy_money) * 10000 //操作资金
        ...
        if (strategy.tList[index].policy_time == '0') {
          ...
          $('.holding-time em.chic').text(strategy.tList[index].policy_time + '天');
          $('.zidong').text('(' + strategy.policy_time_text + ')');
          $('.zbzj').text('(总计=操作资金+服务费+递延费*天数)');
        };
        $('.bond [name="bond"]').val((Number(strategy.tList[index].policy_money) * 10000));
        $('.mingxi').text('（免息时间：' + strategy.fee_time + '天）');
    ...
    tHtmlFn: function () {
      ...
      for (var i = 0; i < strategy.t_policy_leverage.length; i++) {
        var num = Number(strategy.t_policy_leverage[i]) * Number($.trim($('.bond [name="bond"]').val()));
        tHtml = '<span>' + num + '</span>';
        tHtmlData.push(tHtml);
      }
      $('.matching-num').html(tHtmlData.join(''));
      $('.matching-num span').eq(0).addClass('active');
      strategy.bondVal = $('.matching-num span').eq(0).text();
      strategy.caopanzj = $('.matching-num span').eq(0).text();
    ...
    stopLossAndStoplossFn: function () {
      var price = strategy.detailData.margin.buy_price;
      var price1 = strategy.detailData.margin.sell_price;
      var tp = null, tp1 = null, tl = null, tl1 = null, sl_limit_value = null, sl_limit_value1 = null
      var gangan = Number(strategy.bondVal) / Number($('.bond [name="bond"]').val());
      ...
      if (strategy.detailData.market_id == '3' || strategy.detailData.market_id == '5') {
        tp = ((1 + Number(strategy.tp_limit)) * Number(price)).toFixed(3);//多止盈计算
        tp1 = ((1 - Number(strategy.tp_limit)) * Number(price1)).toFixed(3);//空止盈计算
        tl = (Number(price) * (1 - 1 / gangan)).toFixed(3);//多止损计算
        tl1 = (Number(price1) / (1 - 1 / gangan)).toFixed(3);//空止损计算
        sl_limit_value = (Number(price * (1 - strategy.sl_limit))).toFixed(3)
        sl_limit_value1 = (Number(price1 / (1 - strategy.sl_limit))).toFixed(3)
      } else {
        tp = ((1 + Number(strategy.tp_limit)) * Number(price)).toFixed(2);//多止盈计算
        tp1 = ((1 - Number(strategy.tp_limit)) * Number(price1)).toFixed(2);//空止盈计算
        tl = (Number(price) * (1 - 1 / gangan)).toFixed(2);//多止损计算
        tl1 = (Number(price1) / (1 - 1 / gangan)).toFixed(2);//空止损计算
        sl_limit_value = (Number(price * (1 - strategy.sl_limit))).toFixed(2)
        sl_limit_value1 = (Number(price1 / (1 - strategy.sl_limit))).toFixed(2)
      }
    ...
    numberofShares: function () {
      ...
      strategy.gs = 0, strategy.gs1 = 0;
      if (strategy.detailData.market_id == '6' || strategy.detailData.market_id == '7' || strategy.detailData.market_id == '8' || strategy.detailData.market_id == '9' || strategy.detailData.market_id == '10') {
        strategy.gs = Math.floor(Number(strategy.caopanzj) / Number(strategy.detailData.now_price) / Number(strategy.detailData.lots_size))
      } else {
        strategy.gs = Math.floor(Number(strategy.caopanzj) / Number(strategy.detailData.margin.buy_price) / Number(strategy.detailData.lots_size)) * Number(strategy.detailData.lots_size) / strategy.detailData.pip_value;
        strategy.gs1 = Math.floor(Number(strategy.caopanzj) / Number(strategy.detailData.margin.sell_price) / Number(strategy.detailData.lots_size)) * Number(strategy.detailData.lots_size) / strategy.detailData.pip_value;
      }
      $('.bying-rate .gs').text(strategy.gs + '/' + strategy.gs1);
    },
    //服务费计算
    serviceChargeFn: function () {
      var value = Number(strategy.detailData.margin.buy_price * strategy.gs) * Number(strategy.detailData.lots_size) * Number(strategy.detailData.margin.margin_rate),
        value1 = Number(strategy.detailData.margin.sell_price * strategy.gs1) * Number(strategy.detailData.lots_size) * Number(strategy.detailData.margin.margin_rate), //股数*现价
        ...
        guanglifei = (1 + Number(strategy.customer.add_service)) * service_fee * value,
        guanglifei1 = (1 + Number(strategy.customer.add_service)) * service_fee * value1,
        juanshangdaikou = Number((guohu_fee + gui_fee + jianguan_fee) * value),
        juanshangdaikou1 = Number((guohu_fee + gui_fee + jianguan_fee) * value1),
        shouxufei = (1 + Number(strategy.customer.add_buy_commission)) * buy_commission * value,
        shouxufei1 = (1 + Number(strategy.customer.add_buy_commission)) * buy_commission * value1,
        serviceCharge = Number(guanglifei + shouxufei + juanshangdaikou).toFixed(2),
        serviceCharge1 = Number(guanglifei1 + shouxufei1 + juanshangdaikou1).toFixed(2);
      strategy.serviceCharge = serviceCharge
      strategy.serviceCharge1 = serviceCharge1
      if (Number(strategy.detailData.market_id) !== 2) {
        juanshangdaikou = Number((guohu_fee + gui_fee) * value)
        juanshangdaikou1 = Number((guohu_fee + gui_fee) * value1)
      }
    },
    //递延费计算
    deferredChargeFn: function () {
      var delay_fee = Number(strategy.delay_fee);
      var deferredCharge = (Number(strategy.detailData.margin.buy_price) * Number(strategy.gs) * Number(strategy.detailData.lots_size) * delay_fee) * strategy.detailData.margin.margin_rate;
      var deferredCharge1 = (Number(strategy.detailData.margin.sell_price) * Number(strategy.gs1) * Number(strategy.detailData.lots_size) * delay_fee) * strategy.detailData.margin.margin_rate;
      var diyanfeiVal = Number((1 + strategy.customer.add_delay_fee) * deferredCharge).toFixed(2);
      var diyanfeiVal1 = Number((1 + strategy.customer.add_delay_fee) * deferredCharge1).toFixed(2);
      $('.deferred-charge em.diyan').text(diyanfeiVal + '/' + diyanfeiVal1);
    },
    //输入操作资金
    inputFn: function () {
    ...
    totalFn: function () {
      ...
      var fuwufei = strategy.serviceCharge;
      var fuwufei1 = strategy.serviceCharge1;
      var gongji = 0, gongji1 = 0;
      if (Number(strategy.delay_days) > 0) {
        if(strategy.freeze_delayfee==1){
          gongji = baozhengjin + fuwufei;
          gongji1 = baozhengjin + fuwufei1;
        }else{
          gongji = baozhengjin + fuwufei + diyanfeiVal * Number(strategy.delay_days - strategy.fee_time);
          gongji1 = baozhengjin + fuwufei1 + diyanfeiVal1 * Number(strategy.delay_days - strategy.fee_time);
        }
      } else {
        gongji = baozhengjin + fuwufei;
        gongji1 = baozhengjin + fuwufei1
      }
      //var huilv = Number($('.cankaohuilv').text()) || 1;
      $('.total em.gj').text(parseFloat(gongji).toFixed(2) + '/' + parseFloat(gongji1).toFixed(2));
    },
    ...
    purchaseFn: function () {
      $('#jixu-but').click(function () {
        var tp = 0, sl = 0, priceVlaue = 0, buyingQuantityvalue = 0;
        var gangan = Number(strategy.caopanzj) / Number($('.bond [name="bond"]').val());
        if (strategy.cmd == '0') {
          ...
          priceVlaue = strategy.detailData.margin.buy_price
          buyingQuantityvalue = strategy.gs
        } else {
          ...
          priceVlaue = strategy.detailData.margin.sell_price
          buyingQuantityvalue = strategy.gs1
        }
        var data = {
          ...
          assign_volume: $('.shoushu-input').val() || 0,
          policy_type: strategy.id,
          tp: $('.target-profit-name0').val() || 0,
          sl: $('.stop-loss-name0').val() || 0,
          is_demo: '0',
        ...
        success: function (res) {
          if (res.code == '200') {
            var list = res.data.page_data,
              ...
              htmlList, htmlListData = [];
            strategy.mc_list = list,
              zhiHtml = '',
              diyanHtml = '';
            ...
            if (list.length > 0) {
              ...
              for (var i = 0; i < list.length; i++) {
                ...
                if (Number(close_profit) > 0) {
                  close_profit_html = '<em style="color:rgb(40, 240, 100);">' + close_profit + '</em>'
                } else if (Number(close_profit) === 0) {
                  close_profit_html = close_profit
                } else {
                  close_profit_html = '<em style="color:#f00;">' + close_profit + '</em>'
                }
                if (list[i].market_id == '6' || list[i].market_id == '7' || list[i].market_id == '8' || list[i].market_id == '9' || list[i].market_id == '10') {
                ...
                zhiHtml = '<span class="zhiyzhis" style="background-color:yellow;" ' + strategy.styleStr + ' data-type="' + i + '">止盈止损</span>'
                if (list[i].aoto_delay == '1') {
                  diyanHtml = '<span class="guanbidiyan" style="background-color:orange;" ' + strategy.styleStr + ' data-type="' + i + '">关闭递延</span>'
                } else {
                  diyanHtml = ''
                }
                var tbHtml = ''
                if (list[i].suspend_status == '0') {
                  tbHtml = '<span class="xiangqing" ' + strategy.detailBtnStyle + ' data-type="' + i + '"> <a class="loadPage" href="#page/strategyDetails.html?flagid=' + list[i].id + '&outid=1" data-type="' + i + '">详情</a></span><span class="maichu" style="background-color:#00FF7F;" ' + strategy.styleStr + ' data-type="' + i + '" data-cmd="' + list[i].cmd + '">终止合约</span>' + zhiHtml + '' + diyanHtml + ''
                } else {
                  zhiHtml = ''
                  diyanHtml = ''
                  tbHtml = '<span class="xiangqing" ' + strategy.detailBtnStyle + ' data-type="' + i + '"> <a class="loadPage" href="#page/strategyDetails.html?flagid=' + list[i].id + '&outid=1" data-type="' + i + '">详情</a></span><span class="maichu" style="background-color:#00FF7F;" ' + strategy.styleStr + ' data-type="' + i + '" data-cmd="' + list[i].cmd + '">赎回</span>' + zhiHtml + '' + diyanHtml + ''
                }
                var diyanHtml = '';
                if (list[i].policy_time == 0) {
                  if (list[i].aoto_delay == 0) {
                    diyanHtml = "<p style='color:rgb(255,255,41);font-size:12px;'>(" + list[i].policy_delay_time + ")<p>"
                  } else {
                    if (list[i].market_id == '3') {
                      diyanHtml = "<p style='color:rgb(255,255,41);font-size:12px;'>(每天15:40扣除递延费，如可用金额不足，订单将被强制终止合约。)</p>"
                    } else {
                      diyanHtml = "<p style='color:rgb(255,255,41);font-size:12px;'>(每天14:40扣除递延费，如可用金额不足，订单将被强制终止合约。)</p>"
                    }
                  }
                } else {
                  if (list[i].aoto_delay == 0) {
                ...
                htmlList = '<tr ' + classStr + ' >' +
                  '<td>' + tbHtml + '</td>' +
                  '<td>' + id + '</td>' +
                  '<td>' + stock_name + '/' + stock_code + cfdName + '' + diyanHtml + '</td>' +
                  // '<td>' + stock_code + '</td>' +
                  // '<td>' + market_name + '</td>' +
                  '<td>' + cmdName + '</td>' +
                  '<td class="profit_'+id+'">' + close_profit_html + '</td>' +
                  // '<td>' + init_margin.toFixed(2) + '</td>' +
                  '<td>' + left_volume + '</td>' +
                  '<td>' + open_price.toFixed(3) + '</td>' +
                  '<td class="now_price_'+id+'">' + now_price.toFixed(3) + '</td>' +
                  '<td>' + open_time + '</td>' +
                  '</tr>';
              ...
              $('#current-strategy-list tbody').html('<tr class="tr-bg"><td colspan="11">暂无数据</td></tr>');
            }
          }
        }
      })
    ...
    adjustmentTrigger: function () {
      $('.current-strategy-list').on('click', '.zhiyzhis', function () {
        ...
        $('.gpmc-dm span').text(strategy.mc_list[index].stock_name + '/' + strategy.mc_list[index].stock_code);
    ...
    deferredClosure: function () {
      ...
      $('.current-strategy-list').on('click', '.guanbidiyan', function () {
        ...
        $('.guanbi-diyan p').text('将会在' + strategy.mc_list[index].policy_delay_time);
      });
      $('.quxiao-but,.bg').click(function () {
    ...
    sellOutFn: function () {
      ...
      $('.current-strategy-list').on('click', '.maichu', function () {
        ...
        if (cmd == '0') {
          if (strategy.mc_list[index].market_id == '6' || strategy.mc_list[index].market_id == '7' || strategy.mc_list[index].market_id == '8' || strategy.mc_list[index].market_id == '9' || strategy.mc_list[index].market_id == '10') {
            // $('.zhishao').text('(至少卖' + strategy.mc_list[index].sell_min_limit + '手)');
            $('.mcsl input').val(strategy.mc_list[index].left_volume / strategy.mc_list[index].lots_size);
          } else {
        ...
        if (cmd == '1') {
          if (strategy.mc_list[index].market_id == '6' || strategy.mc_list[index].market_id == '7' || strategy.mc_list[index].market_id == '8' || strategy.mc_list[index].market_id == '9' || strategy.mc_list[index].market_id == '10') {
            $('.zhishao').text('(至少买' + strategy.mc_list[index].sell_min_limit + '手)');
            $('.mcsl input').val(strategy.mc_list[index].left_volume / strategy.mc_list[index].lots_size);
          } else {
            $('.zhishao').text('(至少买' + strategy.mc_list[index].sell_min_limit + '股)');
            $('.mcsl input').val(strategy.mc_list[index].left_volume);
          }
        }
        strategy.policy_id = strategy.mc_list[index].id;
        ...
        $('.gumc span').text(strategy.mc_list[index].stock_name + '/' + strategy.mc_list[index].stock_code);
    ...
    sellOutSubmitFn: function () {
      $('#queren').click(function () {
        ajaxPost({
          ...
          data: { policy_id: strategy.policy_id, volume: $('.mcsl input').val(), suspend_status: strategy.suspend_status },
          success: function (res) {
            if (res.code == '200') {
              utils.toast('卖出成功');
              $('.sell-out-box,.bg').fadeOut();
    ...
    siblingsTapFn: function () {
      ...
      $('.strategy-tab li').click(function () {
        ...
        $(this).addClass('active').siblings().removeClass('active');
        $('.table-strategy-list').eq(index).show().siblings().hide();
        if (index == '0') {
          strategy.getMyList({
    ...
    refreshMyList:function(){
        if(strategy.yuming  == 'liangrong-fund.com'||strategy.yuming  == 'boloniasia.com'||strategy.yuming  == 'raytekasia.com'||strategy.yuming == 'acarpsgroup.com'){
          //刷新mylist
          ...
          if(res.code==200){
             ...
             for(var i=0;i<strategy.localList.length;i++){
               for(var j=0;j<res.data.length;j++){
                  if(res.data[j].stock_name==strategy.localList[i].stock_name){
                      //获取报价,计算报价
                      if(strategy.localList[i].cmd==0){
                        quote=(res.data[j].now_price-Number(strategy.localList[i].open_price))*Number(strategy.localList[i].left_volume)*Number(strategy.localList[i].pip_value)
                      }else{
                        quote=(Number(strategy.localList[i].open_price)-res.data[j].now_price)*Number(strategy.localList[i].left_volume)*Number(strategy.localList[i].pip_value)
                      }
                      // console.info('cmd:'+strategy.localList[i].cmd)
                      // console.info('('+res.data[j].now_price+'-'+Number(strategy.localList[i].open_price)+')'+'*'+Number(strategy.localList[i].left_volume)+'*'+Number(strategy.localList[i].pip_value)+'='+quote.toFixed(2))
                      //更新当前价
                      $('.now_price_'+strategy.localList[i].id).text(res.data[j].now_price)
                      //更新盈亏
                      $('.profit_'+strategy.localList[i].id).text(quote.toFixed(2))
                      if(quote.toFixed(2)<0){
                        $('.profit_'+strategy.localList[i].id).css({ 'color': '#28F064' })
                      }else{
                        $('.profit_'+strategy.localList[i].id).css({ 'color': '#FF0000' })
                      }
                  }
               }
             }
          ...
          if (res.code == '200') {
            ...
            if (list.length > 0) {
              for (var i = 0; i < list.length; i++) {
                ...
                htmlList = '<tr ' + classStr + ' >' +
                  '<td>' + id + '</td>' +
                  '<td>' + stock_name + '/' + stock_code + cfdName + '</td>' +
                  // '<td>' + stock_code + '</td>' +
                  '<td>' + market_name + '</td>' +
                  '<td>' + cmdName + '</td>' +
                  // '<td>' + init_margin + '</td>' +
                  '<td>' + volume + '</td>' +
                  '<td>' + open_price + '/' + close_price + '</td>' +
                  // '<td>'+ close_price + '</td>' +
                  '<td>' + huoli + '</td>' +
                  '<td>' + open_time + '</td>' +
                  '<td>' + close_time + '</td>' +
                  '<td><span class="xiangqing" data-type="' + i + '"> <a class="loadPage" href="#page/strategyDetails.html?flagid=' + list[i].id + '&outid=2" data-type="' + i + '">详情</a></span></td>'
                '</tr>';
                htmlListData.push(htmlList);
              }
              if (htmlListData.length > 0) {
                $('#settlement-strategy-list tbody').html(htmlListData.join(''));
                if (page_count > 0) {
                  if (!t.pagination.load) {
              ...
              $('#settlement-strategy-list tbody').html('<tr class="tr-bg"><td colspan="11">暂无数据</td></tr>');
            }
          }
        }
      })
    ...
    sharesTabFn: function () {
      ...
      $('.shares-type-ul').on('click', 'li', function () {
        ...
        $(this).addClass('active').siblings().removeClass('active');
        $('.input-name').html($(this).text() + '<i class="fa fa-angle-down" style="padding-left:5px;"></i>');
        $('.shares-type-ul').slideToggle()
        ...
        success: function (res) {
          if (res.code == '200') {
            ...
            for (var i = 0; i < strategy.SharesTypeVlaue.length; i++) {
              sHtml = '<li data-type="' + strategy.SharesTypeVlaue[i].key + '">' + strategy.SharesTypeVlaue[i].name + '</li>';
              sHtmlData.push(sHtml)
            }
            $('.shares-type-ul').html(sHtmlData.join(''));
            $('.shares-type-ul li').eq(0).addClass('active');
          }
        }
      })
    },
    ...
    seeEchart: function () {
      $('.see-echart').click(function () {
        $('.ec-tab span').eq(0).addClass('active').siblings().removeClass('active');
        $('.echart-con').toggle()
        $('.ma-box').hide()
    ...
    getkline: function () {
      ...
      ajaxPost({
        ...
        data: {
          keyword: stock_code, stock_type: 2,
        ...
        tooltip: {
          ...
          formatter: function (params, ticket, callback) {
            ...
            if (m_datas.price[i] && m_datas.volume[i] !== '0') {
              ...
              html += '<div>涨幅 <span  ' + color + ' >' + strategy.ratioCalculate(m_datas.price[i], m_datas.yestclose) + '%</span></div>';
              html += '<div>成交量 <span  ' + color + ' >' + m_datas.volume[i] + '</span></div></div>'
            } else {
              return
            }
            return html
        ...
        yAxis: [
          {
            ...
            axisLabel: {
              ...
              color: function (val) {
                ...
                return val > m_datas.yestclose ? strategy.upColor : strategy.downColor;
              }
            },
            z: 4,
            splitLine: {
            ...
            axisPointer: {
              ...
              label: {
                formatter: function (params) { //计算右边Y轴对应的当前价的涨幅比例
                  var increase = strategy.ratioCalculate(params.value, m_datas.yestclose)
    ...
    newsTabFn: function () {
      $('.news-tab').on('click', 'li', function () {
        var index = $(this).index();
        $(this).addClass('active').siblings().removeClass('active');
        $('.news-tab-box-s').eq(index).show().siblings().hide();
      })
    },
    // 获取股票和cfd类型导航
    getnavList: function () {
    ...
    //输入手数计算点差二次修改
    inputShouShuFn: function () {
      $('.shoushu-input').bind('input porpertychange', function () {
        strategy.jisuanFn();
      })
    },
    //输入手数计算点差二次修改
    jisuanFn: function () {
      var buy_price = strategy.detailData.margin.buy_price,
        sell_price = strategy.detailData.margin.sell_price,
        margin_rate = strategy.detailData.margin.margin_rate;
      var shoushu = Number($('.shoushu-input').val());
      var shouxufei = Number(strategy.tList[0].buy_commission * shoushu).toFixed(2)
      $('.service-charge span').text(shouxufei);
      var delay_money = Number(strategy.detailData.lots_size * shoushu * buy_price * strategy.tList[0].delay_fee).toFixed(2)
      var delay_money1 = Number(strategy.detailData.lots_size * shoushu * sell_price * strategy.tList[0].delay_fee).toFixed(2)
      $('.diyan1').text(delay_money);
      $('.diyan2').text(delay_money1);
      var total = 0, total1 = 0;
      if (strategy.delay_days == '0') {
        total = Number(Number(buy_price * shoushu * strategy.detailData.lots_size * margin_rate).toFixed(2)) + Number(shouxufei)
        total1 = Number(Number(sell_price * shoushu * strategy.detailData.lots_size * margin_rate).toFixed(2)) + Number(shouxufei)
      } else {
        total = Number(Number(buy_price * shoushu * strategy.detailData.lots_size * margin_rate).toFixed(2)) + Number(shouxufei) + (Number(delay_money) * strategy.delay_days)
        total1 = Number(Number(sell_price * shoushu * strategy.detailData.lots_size * margin_rate).toFixed(2)) + Number(shouxufei) + (Number(delay_money1) * strategy.delay_days)
      }
      $('.gj-box').text(total + '/' + total1);
    },
    // 获取股票和cfd类型导航
    getnavList: function () {
      ajaxPost({
        ...
        success: function (res) {
          if (res.code == '200') {
            var navListData = res.data.value.split(',');
            for (let i = 0; i < navListData.length; i++) {
              if (navListData[i] == '6' || navListData[i] == '7' || navListData[i] == '8' || navListData[i] == '10') {
                $('.gupian-a').show()
              }
              if (navListData[i] == '13') {
                $('.zhishu-type1').show()
